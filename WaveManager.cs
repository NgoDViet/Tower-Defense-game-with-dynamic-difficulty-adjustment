...
namespace TowerDefense.Core
{
    ...
    public class WaveManager : MonoBehaviour
    {
        ...
        [Tooltip("Game difficulty: 0 = Easy, 1 = Normal, 2 = Hard.")]
        [SerializeField] private int difficulty = 0; // Default to Normal
        ...
        public int Difficulty { get => difficulty; set => difficulty = Mathf.Clamp(value, 0, 2); }
        ...
        private IEnumerator SpawnWaveCoroutine(int waveIndex, WaveSetup waveData)
        {
            ...
            // Get quantity multiplier based on difficulty
            float quantityMultiplier = 1.0f;
            if (basicEnemyData != null)
                quantityMultiplier = basicEnemyData.GetQuantityMultiplier(difficulty);

            // Dynamically construct spawn groups from WaveSetup counts with quantity multiplier applied
            List<DynamicSpawnGroup> groups = new List<DynamicSpawnGroup>();

            if (waveData.basicCount > 0 && basicEnemyData != null)
                groups.Add(new DynamicSpawnGroup(EnemyType.Basic, basicEnemyData, Mathf.RoundToInt(waveData.basicCount * quantityMultiplier), waveData.basicSpawnInterval));
            if (waveData.fastCount > 0 && fastEnemyData != null)
                groups.Add(new DynamicSpawnGroup(EnemyType.Fast, fastEnemyData, Mathf.RoundToInt(waveData.fastCount * quantityMultiplier), waveData.fastSpawnInterval));
            if (waveData.tankCount > 0 && tankEnemyData != null)
                groups.Add(new DynamicSpawnGroup(EnemyType.Tank, tankEnemyData, Mathf.RoundToInt(waveData.tankCount * quantityMultiplier), waveData.tankSpawnInterval));
            if (waveData.armorCount > 0 && armorEnemyData != null)
                groups.Add(new DynamicSpawnGroup(EnemyType.Armor, armorEnemyData, Mathf.RoundToInt(waveData.armorCount * quantityMultiplier), waveData.armorSpawnInterval));
        ...
        private IEnumerator SpawnGroupCoroutine(int waveIndex, DynamicSpawnGroup group)
        {
            ...
            for (int i = 0; i < group.count; i++)
            {
                ...
                if (prefabToSpawn != null)
                {
                    ...
                    if (health != null)
                    {
                        health.Initialize(group.enemyData, difficulty);
                    }
