...
namespace TowerDefense.Editor
{
    public class DemoSetupWizard : EditorWindow
    {
        ...
        [MenuItem("Tower Defense/Setup Level2 Spawning")]
        public static void ShowLevel2Window()
        {
            if (EditorApplication.isPlaying)
            {
                EditorUtility.DisplayDialog(
                    "Setup Error",
                    "You cannot setup the Level2 spawning while the editor is in Play Mode. Please exit Play Mode and try again.",
                    "OK");
                return;
            }

            if (EditorUtility.DisplayDialog(
                "Setup Level2 Spawning",
                "This script will add the ObjectPooler, WaveManager, GameManager, UIManager, Canvas UI, and EventSystem to the Level2 scene so that enemies can spawn and follow the path.\n\n" +
                "Do you want to proceed?",
                "Yes, Setup Level2 Spawning",
                "Cancel"))
            {
                SetupLevel2();
            }
        ...
        {
            // Open Level2 scene (checking both name formats)
            string scenePath = "Assets/Scenes/Level 2.unity";
            if (!System.IO.File.Exists(scenePath))
            {
                scenePath = "Assets/Scenes/Level2.unity";
            }
            UnityEngine.SceneManagement.Scene level2Scene = EditorSceneManager.OpenScene(scenePath, OpenSceneMode.Single);

            // Find existing WaypointPath
            WaypointPath pathComp = GameObject.FindObjectOfType<WaypointPath>();
            if (pathComp == null)
            {
                Debug.LogError($"[Setup] Could not find a WaypointPath component in {level2Scene.name}.");
                return;
            }
            ...
            // Save Scene
            EditorSceneManager.SaveScene(level2Scene, level2Scene.path);
            Debug.Log($"[Setup] Saved Scene '{level2Scene.path}'.");
            ...
            {
                if (bScene.path == level2Scene.path)
                {
            ...
            {
                buildScenes.Add(new EditorBuildSettingsScene(level2Scene.path, true));
                EditorBuildSettings.scenes = buildScenes.ToArray();
                Debug.Log($"[Setup] Added scene '{level2Scene.path}' to Build Settings.");
            }
        }
        ...
        {
            // Open Level3 scene (checking both name formats)
            string scenePath = "Assets/Scenes/Level 3.unity";
            if (!System.IO.File.Exists(scenePath))
            {
                scenePath = "Assets/Scenes/Level3.unity";
            }
            UnityEngine.SceneManagement.Scene level3Scene = EditorSceneManager.OpenScene(scenePath, OpenSceneMode.Single);

            // Find existing WaypointPath
            WaypointPath pathComp = GameObject.FindObjectOfType<WaypointPath>();
            if (pathComp == null)
            {
                Debug.LogError($"[Setup] Could not find a WaypointPath component in {level3Scene.name}.");
                return;
            }
            ...
            // Save Scene
            EditorSceneManager.SaveScene(level3Scene, level3Scene.path);
            Debug.Log($"[Setup] Saved Scene '{level3Scene.path}'.");
            ...
            foreach (var bScene in buildScenes)
            {
                if (bScene.path == level3Scene.path)
                {
                    sceneExistsInBuild = true;
                    break;
                }
            ...
            if (!sceneExistsInBuild)
            {
                buildScenes.Add(new EditorBuildSettingsScene(level3Scene.path, true));
                EditorBuildSettings.scenes = buildScenes.ToArray();
                Debug.Log($"[Setup] Added scene '{level3Scene.path}' to Build Settings.");
            }
        }
